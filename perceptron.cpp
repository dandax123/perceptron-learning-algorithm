...
public:
  Perceptron(vector<int> cx_train[SIZE], int cepoch = 10,
             float clearn_rate = 0.2, bool logVerbose = false)
      : weights(cx_train[0].size()) {
  ...
  void train(vector<int> y_train) {
    for (int i = 0; i < epoch; i++) {
      for (int j = 0; j < SIZE; j++) {
        ...
        if (logOutput)
          cout << "Expected: " << y_train[j] << "\tOUTPUT: " << prediction
               << endl;
